...
class REASSURERepair:
    ...
    def point_wise_repair(self, buggy_inputs, output_constraints, core_num=1):
        ...
        arg_list = [
            [buggy_inputs[i], [output_constraints[0][i], output_constraints[1][i]]]
    ...
    def _repair_one_area(self, buggy_input, output_constraint):
        ...
        patch_area = linear_region_from_input(
            buggy_input,
            self.model.allHiddenNeurons(buggy_input).view(-1),
            self.input_boundary,
        )
        A, b = patch_area
    ...
    def repair_via_LP(self, patch_area, output_constraint, linearized_model):
        ...
        A_eq = np.block(
            [
                [
                    dig_block([A.transpose() for _ in range(out_dim)]),
                    dig_block([A.transpose() for _ in range(out_dim)]),
                    np.zeros([in_dim * out_dim, in_cons_num * out_cons_num]),
                    np.zeros([in_dim * out_dim, out_dim]),
                    np.zeros([in_dim * out_dim, 1]),
                ],
                [
                    -np.block(
                        [
                            [A_out[i, j] * A.transpose() for j in range(out_dim)]
                            for i in range(out_cons_num)
                        ]
                    ),
                    np.zeros([in_dim * out_cons_num, in_cons_num * out_dim]),
                    dig_block([A.transpose() for _ in range(out_cons_num)]),
                    np.zeros([in_dim * out_cons_num, out_dim]),
                    np.zeros([in_dim * out_cons_num, 1]),
                ],
            ]
        )
        b_eq = np.block(
            [
                np.zeros(in_dim * out_dim),
                np.block([np.matmul(A_out[i], cf) for i in range(out_cons_num)]),
            ]
        )
        bounds = [None, None]
        A_ub = np.block(
            [
                [
                    dig_block([b for _ in range(out_dim)]),
                    np.zeros([out_dim, in_cons_num * out_dim]),
                    np.zeros([out_dim, in_cons_num * out_cons_num]),
                    np.eye(out_dim),
                    -np.ones([out_dim, 1]),
                ],
                [
                ...
                [
                    np.zeros([out_cons_num, in_cons_num * out_dim]),
                    np.zeros([out_cons_num, in_cons_num * out_dim]),
                    dig_block([b for _ in range(out_cons_num)]),
                    np.block([[A_out[i]] for i in range(out_cons_num)]),
                    np.zeros([out_cons_num, 1]),
                ],
                [
                    -np.eye(in_cons_num * out_dim * 2 + in_cons_num * out_cons_num),
                    np.zeros(
                        [
                            in_cons_num * out_dim * 2 + in_cons_num * out_cons_num,
                            out_dim + 1,
                        ]
                    ),
                ],
        ...
        b_ub = np.block(
            [
                ...
                np.zeros(out_dim),
                np.block(
                    [b_out[i] - np.matmul(A_out[i], df) for i in range(out_cons_num)]
                ),
        ...
        objective = np.block(
            [
                np.zeros(
                    in_cons_num * out_dim * 2 + in_cons_num * out_cons_num + out_dim
                ),
                np.ones(1),
            ]
        )
        ...
        d_x = np.block(
            [
                np.zeros(
                    [out_dim, in_cons_num * out_dim * 2 + in_cons_num * out_cons_num]
                ),
                np.eye(out_dim),
                np.zeros([out_dim, 1]),
            ]
...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        x, x_out = np.round(np.random.random(in_dim), 4), np.round(
            np.random.random(out_dim), 4
        )
        x = np.absolute(x)
